 ...
 * API MARKET DATA: OKX
 *
 * Đã đồng bộ logic tính Entry với Web Dashboard
 * Chỉ số Sức mạnh xu hướng vẫn được tính nội bộ,
 * nhưng KHÔNG hiển thị trên Telegram.
 *
 * KHÔNG còn chu kỳ tự động 120 phút.
 *
 * Bot sẽ:
 *   1. Quét 1 lần khi khởi động.
 *   2. Không tự chạy lại.
 *   3. /scan -> quét thủ công.
 *   4. /ping -> trả OK cho keep-alive.
 * ------------------------------------------------------------
 */
...
const CAPITAL =
  parseFloat(
    process.env.CAPITAL || '1000'
  );

const MIN_CONFIDENCE =
  parseInt(
    process.env.MIN_CONFIDENCE || '32',
    10
  );
...
const SYMBOLS =
  (
    process.env.SYMBOLS ||
    'SOLUSDT,BTCUSDT,ETHUSDT,BNBUSDT,LINKUSDT,SUIUSDT'
  )
    .split(',')
    .map(
      (s) =>
...
function okxInstId(
  ...
  if (
    ...
    throw new Error(
      `Symbol không hợp lệ cho OKX: ${symbol}`
    );
  }
...
async function fetchJSON(
  ...
  if (!data) {
    ...
    throw new Error(
      `OKX trả về dữ liệu JSON không hợp lệ từ ${url}`
    );
  }
...
async function fetchOKXMarketData(
  ...
  const tickerData =
    await fetchJSON(
      `${OKX_BASE_URL}/api/v5/market/ticker?instId=${encodeURIComponent(
        instId
      )}`
    );
  ...
  if (
    !tickerData.data ||
    !tickerData.data.length ||
    !tickerData.data[0].last
  ) {

    throw new Error(
      `OKX không trả giá hiện tại cho ${instId}`
    );
  }
  ...
  const candleData =
    await fetchJSON(
      `${OKX_BASE_URL}/api/v5/market/candles?instId=${encodeURIComponent(
        instId
      )}&bar=4H&limit=300`
    );
  ...
  if (
    ...
    throw new Error(
      `OKX không trả đủ dữ liệu nến 4H cho ${instId}: ${
        Array.isArray(
          candleData.data
        )
          ? candleData.data.length
  ...
  // để giữ nguyên logic EMA/ATR/Regression.
  ...
  if (
    ...
    throw new Error(
      `OKX không có đủ nến hợp lệ cho ${instId}: ${candles.length}`
    );
  }
...
function buildTelegramMessage(
  ...
  lines.push(
    `${trendEmoji(
      trendInfo.trendLabel
    )} <b>${config.name} (${symbol})</b> — <b>${trendInfo.trendLabel}</b> (Độ tin cậy: ${trendInfo.confPercentNum}%)`
  );
  ...
  lines.push(
    `📐 Ngưỡng tiêu chuẩn: ${trendInfo.trendThreshold.toFixed(
      2
    )}%`
  );
  ...
  lines.push(
    `🎯 <b>Khuyến nghị:</b> ${actionRecommendation(
      trendInfo
    )}`
  );
...
async function runScanCycle() {
  ...
  for (
    ...
    // Giữ khoảng nghỉ 800ms
    // giữa từng coin.
    // Đây KHÔNG phải chu kỳ 120 phút.
...
(async () => {
  ...
  // Chỉ quét 1 lần khi bot khởi động.
  await runScanCycle();
  ...
  // );
  //
  // Vì vậy bot sẽ không tự quét mỗi 120 phút nữa.
